...
const OpenedMessageColumn = ({ projectPoolAddress }: Props) => {
  ...
  const { userPoolAddress, handleUserApproveCherToProjectPool, userMining } = useUserPoolContract({ userOwnerAddress });
  const { daoPoolAddress, handleDaoApproveCherToProjectPool, daoMining } = useDaoPoolContract({ daoOwnerAddress });
